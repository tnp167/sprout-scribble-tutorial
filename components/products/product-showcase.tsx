...
export default function ProductShowCase({
  ...
  return (
    <Carousel setApi={setApi} opts={{ loop: true }} className="relative flex-1">
      <CarouselContent>
        {variants.map(
          (variant) =>
        ...
        {variants.map(
          (variant) =>
            ...
            variant.variantImages.map((img, index) => {
              return (
                <div className="flex overflow-clip py-2 gap-4" key={img.url}>
                  {img.url ? (
                    <Image
                      ...
                      className={cn(
                        index === activeThumbnail[0]
                          ? "opacity-100"
                          : "opacity-75",
                        "rounded-md transition-all duration:300 ease-in-out cursorpointer hover:opacity-75"
                      )}
                      width={72}
                      height={48}
                      src={img.url}
